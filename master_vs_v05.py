...
#
#==================================================================================================
def mysearch(lookup_lst,lookup_value,return_type):
    #THIS MIGHT BE DONE WITH INDEX.  MIGHT NOT NEED TO USE RANGE
    # return_type:
    #  0 will return true or false. 
    #  1 will return location of 1st instance in list. 
    
    lng=len(lookup_lst)
    n=0
    
    for i in range(len(lookup_lst)):
        if lookup_lst[i] == lookup_value:    
            my_return=n
            break
        n+=1    
    if return_type==0:
...
#==================================================================================================
# because everything is a string if you use the type function it will only return string
# returns type: myint, myfloat, mystring or mypunc. if input = 0 then returns myint
#==================================================================================================
def my_type(char_test): 
    try:
        input=float(char_test)
        if char_test==0:
            return 'myint'
        elif int(char_test)/char_test==1:
            return 'myint'
        else:    
            return 'myfloat'
    except ValueError:
        if char_test in LST_PUNC:
            return 'mypunc'
        else:
            return 'mystr'
    return 'mystr'
...
#==================================================================================================
# If string contains only numbers return True
#==================================================================================================
def my_isnbr(char_test): 
    try:
        char_test=float(char_test)
        return True
    except ValueError:
        return False
#==================================================================================================
# If string contains only puncuation return True
#==================================================================================================
def my_ispunc(char_test): 
    try:
        if char_test in LST_PUNC:
            return True
        else:
        ...
        return False
#==================================================================================================
# if string contains only alpha return True
#==================================================================================================
def my_isstr(char_test): 
    try:
        if char_test in LST_PUNC:
            return False
        elif char_test.isalpha():
            return True
        else:
            return False
    except ValueError:
        return False
#==================================================================================================
# If string is a date return True. will only work for one date formate: yyyy-mm-day
#==================================================================================================
def my_isdate(char_test):     
    try:
        year, month, day=char_test.split('-')
        datetime.datetime(int(year),int(month),int(day))
        return True
    except ValueError:
        return False
#==================================================================================================
# determine is selenium already has a chrome browser open so we don't repeatadly login.  in notebook
# the browser won't close on it's own, but in vscode the browser auto closes after the code is complete
# NEED TO FIGURE OUT HOW TO KEEP IT OPEN.
#==================================================================================================
def so_is_open():
    try:
